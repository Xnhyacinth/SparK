...
if __name__ == '__main__':
    ...
    for dataset in dataset_list:
        ...
        for idx, method in enumerate(["full_kv", 'snap_adathink']):
                ...
                if compress_q is not None:
                    if 'snap_adathink' in method:
                        ...
                        if pooling_ratio is not None:
                            # breakpoint()
                            eval_file = os.path.join(args.results_dir, 'comress_questions', args.tem, args.compress_ratio, 'longbench', dataset, f"{method}__max_context{max_context_length}__no{pooling_ratio}__channel{args.com_channel}.json")
                        else:
                            eval_file = os.path.join(args.results_dir, 'comress_questions', args.tem, args.compress_ratio, 'longbench', dataset, f"{method}__max_context{max_context_length}__channel{args.com_channel}.json")
                        # breakpoint()
                    elif 'think' in method:
                        eval_file = os.path.join(args.results_dir, 'comress_questions', args.tem, args.compress_ratio, 'longbench', dataset, f"{method}__max_context{max_context_length}__channel{args.com_channel}.json")
                    elif 'full_kv' in method:
                        eval_file = os.path.join(args.results_dir, 'comress_questions', args.tem, '0', 'longbench', dataset, f"{method}__max_context{max_context_length}.json")    
                    else:
                        eval_file = os.path.join(args.results_dir, 'comress_questions', args.tem, args.compress_ratio, 'longbench', dataset, f"{method}__max_context{max_context_length}.json")
                else:
                    if 'snap_adathink' in method:
                        if pooling_ratio is not None:
                            eval_file = os.path.join(args.results_dir, args.tem, args.compress_ratio, 'longbench', dataset, f"{method}__max_context{max_context_length}__no{pooling_ratio}__channel{args.com_channel}.json")
                        else:
                            eval_file = os.path.join(args.results_dir, args.tem, args.compress_ratio, 'longbench', dataset, f"{method}__max_context{max_context_length}__channel{args.com_channel}.json")
                    elif 'think' in method:
                        eval_file = os.path.join(args.results_dir, args.tem, args.compress_ratio, 'longbench', dataset, f"{method}__max_context{max_context_length}__channel{args.com_channel}.json")
                    elif 'full_kv' in method:
                        eval_file = os.path.join(args.results_dir, args.tem, '0', 'longbench', dataset, f"{method}__max_context{max_context_length}.json")    
                    else:
                        eval_file = os.path.join(args.results_dir, args.tem, args.compress_ratio, 'longbench', dataset, f"{method}__max_context{max_context_length}.json")
