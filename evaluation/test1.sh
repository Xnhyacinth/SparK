...
output_prefix=/modelopsnas/modelops/468440/kvpress/logs0/${model}
...
for compression_ratio in "${compression_ratios[@]}"; do
  (
    for data_dir in $dataset_list; do
      echo "Running press_name: $press on dataset ${data_dir} with compression_ratio: $compression_ratio on GPU cuda:$i"
      CUDA_VISIBLE_DEVICES=${gpus} python -u eval0.py --dataset $dataset --data_dir $data_dir --model $model_name_or_path --press_name $press --max_capacity_prompt $compression_ratio --threshold_ratio ${threshold_ratio} --pooling_ratio ${pooling_ratio} --mode ${mode} --key_channel_compression_ratio ${key_channel_compression_ratio} --temperature ${temperature} --device "auto" --save_dir /modelopsnas/modelops/468440/kvpress/output000_${pooling_ratio}_${threshold_ratio} ${extra_args}
    done
