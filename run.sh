...
for com in "${coms[@]}"
  ...
  for ratio in "${ratios[@]}"
    ...
    python metric.py --results_dir /modelopsnas/modelops/468440/kvpress/output_norm1/results/Meta-Llama-3-8B-Instruct --compress_ratio ${ratio} --compress_q com --tem 0.0 --com_channel ${com} #--threshold_ratio ${xxx} #--pooling_ratio ${xxx}
    done 
  done
