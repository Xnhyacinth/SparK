...
def evaluate(
    ...
    for d in tqdm(ds):
        ...
        max_new_tokens_ = max_new_tokens if max_new_tokens is not None else d["max_new_tokens"]
    ...
    with open(f"{'/'.join(str(save_filename).split('/')[:-1])}/res.json", "a") as f:
        json.dump(f'{press_name}_{key_channel_compression_ratio}_{threshold_ratio}: {metrics}', f, indent=4)
        f.write('\n')
